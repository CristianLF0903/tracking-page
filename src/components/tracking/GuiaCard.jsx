...
const GuiaCard = ({ guia }) => {
	...
	const shipping_status =
		statusMap[_fulfillment_status] || _fulfillment_status || 'Preparando'

	const parseProducts = (productsPayload) => {
		const normalizePayload = (payload) => {
			...
			if (typeof payload === 'string') {
				try {
					...
					console.error('Error parsing _products JSON:', e, payload)
					return []
				}
			}
			...
			.flatMap((item) => {
				...
				if (typeof item === 'string') {
					try {
						...
						console.error('Error parsing product item JSON:', e, item)
						return []
					}
				}
				return [item]
	...
	const products = parseProducts(_products)
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-sm text-secondary font-medium">
							{_tracking_number === 'PENDIENTE'
								? 'Tu pedido está siendo procesado por Madecentro'
								: _tracking_company}
						</p>
					</div>
				</div>
