...
const OrderCard = ({ order }) => {
  ...
  const customerName = `${_customer_first_name} ${_customer_last_name}`.trim() || 'Cliente No Identificado';
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <h2 className="text-2xl font-bold text-secondary-dark">Resumen de tu pedido</h2>
            ...
            <div>
              ...
              <div className="space-y-1.5 mt-2">
                <div className="flex items-center gap-1.5 text-secondary/60">
                  <Phone size={12} />
                  <span className="text-xs font-medium">{_customer_phone || "N/A"}</span>
                </div>
                {_customer_email && (
                  <div className="flex items-center gap-1.5 text-secondary/60">
                    <Mail size={12} />
                    <span className="text-xs font-medium truncate max-w-[150px]">{_customer_email}</span>
                  </div>
                )}
              </div>
            </div>
