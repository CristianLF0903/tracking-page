  ...
  if (error) {
    return (
      <div className="py-8">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-primary hover:text-primary-dark font-semibold mb-8 transition-standard group"
        >
          <ArrowLeft
            size={18}
        ...
        {error.includes("No se encontró") ? (
          ...
          <ErrorState
            message={error}
            onRetry={() => fetchTracking(decodedId)}
          />
        )}
      </div>
    );
